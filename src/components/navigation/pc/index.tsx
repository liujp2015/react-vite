...
const Index: React.FC<PropType> = ({ categories }) => {
  ...
  return (
    <div className=" bg-white dark:bg-zinc-800 sticky top-0 left-0 w-full z-10 h-10">
      <ul
      ...
      >
        ...
        >
          <SvgIcon
            ...
            className="w-1 h-1 fill-zinc-900 dark:fill-zinc-300"
          ></SvgIcon>
        </div>
        {categories?.map((item) => (
          <li
...
cursor-pointer hover:bg-zinc-200 rounded mr-1 mb-1 dark:text-zinc-500 dark:hover:bg-zinc-900 "
          >
            {item.name}
          </li>
        ))}
